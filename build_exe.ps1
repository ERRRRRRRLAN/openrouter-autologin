...
Write-Host "== 1/6 prepare sea-config.json ==" -ForegroundColor Cyan
@'
{
  "main": "launcher.js",
...
Write-Host "== 4/6 remove the old signature (postject needs it) ==" -ForegroundColor Cyan
$sigtool = Get-Command signtool -ErrorAction SilentlyContinue
if ($sigtool) { & $sigtool.Source remove /s launcher.exe } else { Write-Host "signtool not found - usually fine for official node.exe" }
